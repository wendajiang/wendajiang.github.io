...
[[assignment_statements|assignment_statements|assignment statements]]  always computes the updated value and schedules the update as an NBA update event, either in the current time step if the delay is zero or as a future event if the delay is nonzero. The values in effect when the udpate is placed in the event region are used to compute both the right-hand value and the left-hand target.
