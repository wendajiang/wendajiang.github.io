---
title: SQLite
date: 2026-03-12 22:50
tags:
  - SQL
---
# how to choose right database engine
1. **Is the data separated from the application by a network? → choose client/server**
    ...
    _Nota Bene:_ In this rule, "application" means the code that issues SQL statements. If the "application" is an [application server](https://sqlite.org/whentouse.html#serversidedb) and if the content resides on the same physical machine as the application server, then SQLite might still be appropriate even though the end user is another network hop away.
